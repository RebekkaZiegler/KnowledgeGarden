const APP_VERSION = "1.0.33";   // ← bump this with every push
...
function reRenderLabelExercise() {
  ...
  area.innerHTML = `<div class="re-question-panel re-question-panel--label">
    ...
    <div class="label-diagram-wrapper" style="width:min(100%,calc(48vh * ${(ex.aspectRatio || '5/4').split('/')[0]} / ${(ex.aspectRatio || '5/4').split('/')[1]}))">${
      ex.diagramType === "image"
        ? `<img src="${ex.imagePath}" style="display:block;width:100%;height:auto">`
        : (ex.svgContent || "")
    }${zoneHtml}</div>
