...
/* ══════════════════════════════════════════════════════════
   ...
   collects — automatically, no further input — while it happens to be
   passing over a column whose current bottom output matches its color;
...
// Advances every active container's belt position by dtMs, and — for any
// container currently aligned with a column whose bottom-most remaining
// cell matches its color, provided its own per-container collection
// cooldown has elapsed — removes that one cell into it. MUTATES
// state.columns/state.containers directly (this runs every animation
// frame; performance matters here unlike every other mini-game's
// occasional full-rebuild-on-tap model). Returns the column indices any
// pixel was actually collected from this tick, for the caller's visual
// feedback (empty array in the common case where nothing aligned).
function msTick(state, dtMs) {
  ...
  for (const c of state.containers) {
    ...
    if (c.filled < c.capacity && c.msSinceCollect >= state.collectIntervalMs) {
      const colIdx = Math.floor(c.beltPos);
...
// Removes the given (col, rowFromBottom=0) pixel from the cached picture
// ImageData and blits just that update — cheap regardless of picture size.
function msErasePixel(colIdx, rows, cols) {
  if (!msPictureCtx || !msPictureImageData) return;
  const r = rows - 1; // the bottom-most display row for this column (rowFromBottom 0)
  const i = (r * cols + colIdx) * 4;
  msPictureImageData.data[i + 3] = 0;
  msPictureCtx.putImageData(msPictureImageData, 0, 0, colIdx, r, 1, 1);
}
  ...
  const frame = (t) => {
    ...
    if (dtMs > 0 && ms.containers.length) {
      ...
      if (removedCols.length) {
        removedCols.forEach(colIdx => msErasePixel(colIdx, level.rows, level.cols));
        saveState();
